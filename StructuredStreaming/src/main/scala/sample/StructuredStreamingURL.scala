...
object StructuredStreamingURL {
  def main(args: Array[String]):Unit = {
    ...
    val websiteCountDF = logsDF.withColumn("eventTime", current_timestamp)

    val endPoints = websiteCountDF.groupBy(window(col("eventTime"), "30 seconds", "10 seconds"), col("endpoint")).count()

    val sortedEndpoints = endPoints.orderBy(col("count").desc)
    val query = sortedEndpoints.writeStream.outputMode("complete").format("console").queryName("counts").start()
